...
class EvalHelper:
    ...
    def checkpoint_results(self, results):
        ...
        """
        results.to_csv(
            self.params.result.dir + self.params.model.name + "_" + self.params.data.name + ".rst",
    ...
    def perform_eval(self):
        ...
        results = pd.DataFrame(columns=["fold"])
        rankings = {}
        thresholds = [1, 5, 10]
        ...
        for fold in self.params.data.folds:
            ...
            for k in thresholds:
                results.at[fold, f"MRR@{k}"] = round(self.mrr_at_k(ranking.values(), k, len(ranking)), 3)
                results.at[fold, f"RCL@{k}"] = round(self.recall_at_k(ranking.values(), k, len(ranking)), 3)
        ...
        self.checkpoint_results(results)
